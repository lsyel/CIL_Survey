...
class MoELayer(nn.Module):
    ...
    def visualize_attention(self, save_path=None):
        """可视化特征注意力权重"""
        if not hasattr(self, "feature_attention"):
            return
        
        # 获取第一层注意力权重
        weights = self.feature_attention[0].weight.data.cpu().numpy()
    ...
    # ===== 新增诊断工具：路由记忆相似度报告 =====
    def report_memory_similarity(self):
        """报告专家记忆之间的相似度"""
        memory = self.routing_memory.data.cpu().numpy()
        similarities = np.zeros((self.num_experts, self.num_experts))
        ...
        for i in range(self.num_experts):
            for j in range(self.num_experts):
                if i == j:
                    ...
                    sim = np.dot(memory[i], memory[j]) / (
                        np.linalg.norm(memory[i]) * np.linalg.norm(memory[j]) + 1e-8
                    )
...
class EnhancedGateNetwork(nn.Module):
    def __init__(self, input_dim, num_experts, hidden_dim=512, num_layers=3):
        super().__init__()
        ...
        # 输出层
        self.layers.append(nn.Linear(hidden_dim, num_experts))
